...
async fn main() {
    ...
    let net_config = config.network_monitor;
    let node_config = config.node_monitor;
        ...
        loop {
            ...
            for (pid, process) in sys.processes() {
                ...
                if  node_config.include_users.contains(&user_name.to_string()) && !node_config.exclude_users.contains(&user_name.to_string()) {
                    ...
                    if process_run_time > node_config.run_time{
                        warn!(
                            "Warning: User: {:?} Process {} ({}) has been running for more than {} seconds, already elpased: {} seconds",
                            user_name,
                            pid,
                            process_cmd.join(" "),
                            node_config.run_time,
                            process_run_time
                        );
                    ...
                    if process.cpu_usage() >= res_config.cpu_threshold || process.memory() >= res_config.memory_threshold || process.virtual_memory() >= res_config.virtual_memory_threshold {
                        ...
                        msg_content.push_str(&format!("Warning: User: {:?} Process {} ({}): CPU usage: {}%, Memory usage: {}, Virtual Memory: {}\n", user_name, pid, process_cmd.join(" "), process.cpu_usage(), format_bytes(process.memory()), format_bytes(process.virtual_memory())));
                    }
                    ...
                    if iops > res_config.iops_threshold {
                        warn!(
                            "Warning: User: {:?} Process {} ({}) has high IOPS: {} bytes, read: {} bytes, write: {} bytes",
                            user_name,
                            pid,
                            process_cmd.join(" "),
                            format_bytes(iops),
                        ...
                        msg_content.push_str(&format!("Warning: User: {:?} Process {} ({}) has high IOPS: {} bytes, read: {} bytes, write: {} bytes\n", user_name, pid, process_cmd.join(" "), format_bytes(iops), format_bytes(read_bytes), format_bytes(write_bytes)));
                    }
                }
            }
            ...
            for (interface_name, network) in &networks {
                if network.received() > net_config.data_threshold || network.transmitted() > net_config.data_threshold {
                    ...
                    msg_content.push_str(&format!("Warning: {} received {}, transmitted {}\n", interface_name, format_bytes(network.received()), format_bytes(network.transmitted())));
            ...
            // let mut tcp_msg_title = format!("--------------------------------TCP--------------------------------\n");
            // get the tcp table
            ...
            // tcp_msg_title.push_str(&format!("{:<26} {:<26} {:<15} {:<8} {}\n", "Local address", "Remote address", "State", "Inode", "PID/Program name"));
            // let mut tcp_msg = String::new();
